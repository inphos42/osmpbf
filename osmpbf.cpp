...
bool OSMpbf::readOSMData(const char * rawdata, uint32_t size) {
	...
	for (int i = 0; i < primitiveBlock.stringtable().s_size(); i++) {
		cout << '[' << i << "] " << primitiveBlock.stringtable().s(i) << endl;
	}
	...
	for (it = primitiveBlock.primitivegroup().begin(); it != primitiveBlock.primitivegroup().end(); ++it) {
		...
		if (it->has_dense()) {
			...
			cout << "key-val-0 dump:" << endl;
			for (int j = 0; j < it->dense().keys_vals_size(); j++) {
				if (it->dense().keys_vals(j))
					cout << '[' << it->dense().keys_vals(j) << ']';
				else
					cout << '-';
				cout.flush();
			}
